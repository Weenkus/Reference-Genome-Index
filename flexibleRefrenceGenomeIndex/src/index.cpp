...
//		printTree(root);
		...
		printf("\nLoding data structures from the saveStructures folder.\n\n");

		// SA deserialization
		int *SA, inputLength = 0;
		if (deserializeSA(&SA, &inputLength) == 1)
			printf("Suffix array loaded.\n");
		else
			printf("Problem while loading the suffix array.\n");
...
//		printf("\n\n");
		...
		// BWT deserialization
		char *BWT;
		if (deserializeBWT(&BWT) == 1)
			printf("Burrows-Wheeler transform loaded.\n");
		else
			printf("Problem while loading the Burrows-Wheeler transform.\n");

//		printf("BWT: ");
//		puts(BWT);
//		printf("\n");

		// Alphabet deserialization
		char *alphabet;
		int alphabetLength = 0;
		if (deserializeAlphabet(&alphabet, &alphabetLength) == 1)
			printf("Alphabet loaded.\n");
		else
			printf("Problem while loading the alphabet.\n");

//		printf("Alphabet: ");
//		puts(alphabet);
//		printf("\n");

		// Prefix sum table - C deserialization
		int *C;
		if (deserializeC(&C) == 1)
			printf("Prefix sum table - C loaded.\n");
		else
			printf("Problem while loading the prefix sum table - C.\n");

//		printf("Prefix sum table: ");
//		for (int i = 0; i < IUPAC_SIZE + 1; i++)
//			printf("C[%c] = %d, ", artificialAlphabet[i], C[i]);
//		printf("\n\n");

		// Wavelet tree - OCC deserialization
		node *root;
		if (deserializeWaveletTree(&root) == 1)
			printf("Wavelet tree - OCC loaded.\n");
		else
			printf("Problem while loading the wavelet tree - OCC.\n");

		printf("\nAll data structures loaded successfully.\n");
//		printTree(root);

		// Ask the user if he wants to serach the newly loaded index
		char userInput;
		printf("\n");
		do {
			printf("Enter a pattern you want to search: ");
			char pattern[IUPAC_SIZE + 1];
			scanf("%s", pattern);
			int occ = patternSearch(pattern, root, C, inputLength, alphabet, alphabetLength);
