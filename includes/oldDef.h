...
#define getBitOld(position) (bitovi[(position)/8]&maskaOld[(position)%8] ? 1 : 0)

// Postavi bit na zadanoj poziciji
#define setBitOld(position, bit) ((bit)== 1 ? bitovi[(position)/8]|maskaOld[(position)%8] : bitovi[(position)/8]&(~(maskaOld[(position)%8])))

// Dobi rank odgovarajuceg bita
#define getRankBitOld(position) (bitovi[0]&maskaOld[(position)%8] ? 1 : 0)
